# Firestore HTTPS Perl Server

A simple Model Context Protocol (MCP) server implemented in Perl using `MCP::Server` and `Mojolicious`. This server is designed to communicate over `stdio` and serves as a foundational example for Perl-based MCP integrations interacting with Firestore via HTTPS.
...
```json
{
  "mcpServers": {
    "perl-hello-world": {
      "command": "perl",
      "args": [
        "-I/path/to/firestore-https-perl/local/lib/perl5",
        "/path/to/firestore-https-perl/server.pl"
      ]
    }
  }
}
