...
This project provides a basic MCP server named `hello-world-server` that exposes a single tool: `greet`. It writes structured JSON logs to stderr, ensuring that the stdout stream remains clean for the MCP protocol JSON-RPC messages.
...
```json
{
  "mcpServers": {
    "c-hello-world": {
      "command": "/absolute/path/to/mcp-stdio-c/server",
      "args": []
    }
  }
}
