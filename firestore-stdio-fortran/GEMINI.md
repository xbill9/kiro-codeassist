...
This is a **Fortran-based Model Context Protocol (MCP) server** named `firestore-stdio-fortran`, implemented using the `mcpc` library via C bindings (`iso_c_binding`). It functions as a **Firestore Inventory Management System**, exposing tools to list, query, seed, and reset an inventory database stored in Google Cloud Firestore.
...
*   `server.f90`: The main entry point. Defines the MCP server, registers tools, and bridges to C implementations.
*   `firestore_client.c`: C implementation of the Firestore REST client. Uses `popen` to call `curl` and `gcloud`. Parses JSON responses.
*   `c_helpers.c`: C helper functions for `stdio` handling and bridging macros.
*   `Makefile`: Build configuration.
*   `test_server_fortran.py`: Python script for integration testing.
...
*   **Server Initialization:** `mcpc_server_new_iostrm`
...
*   `make firestore-server`: Builds the Fortran server binary, linking `server.o`, `c_helpers.o`, and `firestore_client.o`.
