...
def test_server():
    print("Testing 'firestore-stdio-zig' server...")
    ...
    try:
        ...
        if 'get_root' not in tools:
            print(f"Error: 'get_root' tool not found in list. Found: {tools}")
            sys.exit(1)
        print("✓ tools/list successful")

        # Step 4: Test tools/call (get_root)
        send(call_req)
        res = receive()
        if not res:
        ...
        if "Cymbal Superstore" not in text:
