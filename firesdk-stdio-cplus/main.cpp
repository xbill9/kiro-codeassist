...
int main() {
  ...
  if (!PROJECT_ID.empty()) {
    ...
    try {
        ...
        // db_client->SetToken(ACCESS_TOKEN); // No longer needed with SDK
        
        if (db_client->CheckConnection()) {
          log_json("INFO", "Firestore connection successful");
          db_running = true;
        } else {
          db_error = "Firestore connection failed to check";
          log_json("ERROR", db_error);
        }
    } catch (const std::exception& e) {
        db_error = std::string("Failed to initialize Firestore Client: ") + e.what();
        log_json("ERROR", "Failed to initialize Firestore Client", {{"error", e.what()}});
    }

  } else {
    db_error = "Project ID not found. Set GOOGLE_CLOUD_PROJECT or FIREBASE_PROJECT_ID.";
    log_json("ERROR", db_error);
  }

  StdioServer server("firesdk-stdio-cplus", "1.0.0");
  ...
  server.register_tool(
      ...
      [](const json &, const std::string &) -> json {
         ...
         json msg;
         msg["type"] = "text";
         msg["text"] = db_running ? "Connected" : ("Not connected: " + db_error);
         content.push_back(msg);
         return content;
      });
