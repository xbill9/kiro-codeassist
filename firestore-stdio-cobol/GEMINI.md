...
This is a **COBOL-based Model Context Protocol (MCP) server** named `firestore-stdio-cobol`, implemented using the `mcpc` library via C bindings. It functions as a **Firestore Inventory Management System**, exposing tools to list, query, seed, and reset an inventory database stored in Google Cloud Firestore.
...
*   `make firestore-server`: Builds the COBOL server binary.
