...
actor MetadataServerTokenProvider: GoogleAuthProvider {
  ...
  func getAccessToken() async throws -> String {
    ...
    guard response.status == .ok else {
      ...
      throw NSError(
        domain: "GoogleAuth", code: Int(response.status.code),
        userInfo: [NSLocalizedDescriptionKey: "Metadata server error: \(String(buffer: body))"])
    }
  ...
  func getProjectId() async throws -> String {
    ...
    guard response.status == .ok else {
      throw NSError(
        domain: "GoogleAuth", code: Int(response.status.code),
        userInfo: [NSLocalizedDescriptionKey: "Failed to fetch project ID from metadata server"])
    }
