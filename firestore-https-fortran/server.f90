module mcpc_interface
    ...
    interface
        ...
        ! MCPC Functions
        function mcpc_server_new_tcp() bind(c, name="mcpc_server_new_tcp")
            import :: c_ptr
            type(c_ptr) :: mcpc_server_new_tcp
        end function mcpc_server_new_tcp
...
program server
    ...
    call log_info_c(c_char_"Server starting..." // c_null_char)

    ! Initialize Server
    server_ptr = mcpc_server_new_tcp()
    ...
    ! Set the server name
    ret = mcpc_server_set_nament(server_ptr, c_char_"firestore-https-fortran" // c_null_char)
    if (ret /= 0) then
...
contains
    ...
    function setup_tools(server) result(res)
        ...
        tool = mcpc_tool_new2(c_char_"greet" // c_null_char, &
            c_char_"Get a greeting from a local http server." // c_null_char)
