...
This is a **Perl-based Model Context Protocol (MCP) server** using the `MCP::Server` module. It is designed to expose tools (like `greet`) over HTTP for integration with MCP clients (such as Claude Desktop or Gemini clients).
...
*   `Makefile`: Development shortcuts (install, run, test, lint, format).
...
*Note: You need to keep this server running for the MCP client to connect.*
