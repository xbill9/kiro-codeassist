...
This project provides an MCP server named `mcp-https-cobol` (binary: `server-cobol`) that exposes a single tool: `greet`. It listens on a TCP port (default 8080) for JSON-RPC messages over HTTP.
