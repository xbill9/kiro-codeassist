...
mcpServer.registerTool(
  "search",
  {
    ...
    inputSchema: z.object({
      query: z.string().describe("The search query to filter products by name")
    }) as any,
  },
  async (args: any) => {
    ...
    try {
        const products = await firestore.collection("inventory").get();
        const allProducts = products.docs.map(docToProduct);
        const results = allProducts.filter(p => p.name.toLowerCase().includes(query.toLowerCase()));
        return { content: [{ type: "text" as const, text: JSON.stringify(results, null, 2) }] };
    } catch (e) {
        logger.error("Error searching products:", e);
        return { content: [{ type: "text" as const, text: "Error searching products." }], isError: true };
    }
  }
);
...
app.get("/health", (req: Request, res: Response) => {
  res.status(200).json({ message: "Healthy", db: dbRunning, hostname: os.hostname() });
});
